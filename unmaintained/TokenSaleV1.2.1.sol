...
contract TokenSale is FinalizableCrowdsale, Pausable {
    ...
    uint256 public starRatePer100000;
     ...
     * @param newStarRatePer100000 Figure that corresponds to the new STAR rate per token
     */
    function setStarRate(uint256 newStarRatePer100000) external onlyOwner {
        require(newStarRatePer100000 != 0, "Star rate must be more than 0!");

        emit TokenStarRateChanged(starRatePer100000, newStarRatePer100000);
        starRatePer100000 = newStarRatePer100000;
    }
        ...
        isWhitelisted(beneficiary)
    {
        ...
        if (starAllocationToTokenSale > 0) {
            ...
            uint256 tokens = starAllocationToTokenSale.mul(starRatePer100000).div(100000);

            // remainder logic
            if (tokensSold.add(tokens) > crowdsaleCap) {
                tokens = crowdsaleCap.sub(tokensSold);

                starAllocationToTokenSale = tokens.div(starRatePer100000).div(100000);
            }
