...
contract("TokenSale", ([owner, wallet, buyer, buyer2, user1, fakeWallet]) => {
  ...
  const isNOTMinting = false;

  let startTime, endTime;
  let crowdsale, token, star, whitelist;
  let crowdsaleTokensLeftover;

  const newCrowdsale = async (rate, starRate, isWeiAccepted = true, isMinting = true) => {
  ...
  describe("when NOT Minting", () => {
    beforeEach("initialize contract", async () => {
      await newCrowdsale(rate, starRate, isWeiAccepted, isNOTMinting);
    ...
    it("deployment fails when both starRate and rate are zero", async () => {
      try {
        await newCrowdsale(0, 0, isWeiAccepted, isNOTMinting);
        assert.fail();
      } catch (error) {
        ensuresException(error);
      }
    ...
    it("deployment succeds when either starRate or rate are set", async () => {
      await newCrowdsale(rate, 0, isWeiAccepted, isNOTMinting);
      // deployment without starRate
      (await crowdsale.starRate()).should.be.bignumber.eq(0);

      // deployment without rate
      await newCrowdsale(0, starRate, false, isNOTMinting);
      (await crowdsale.rate()).should.be.bignumber.eq(0);
    });
    ...
    describe("token purchases", () => {
      ...
      it("sends STAR raised to wallet", async () => {
        await newCrowdsale(crowdsaleCap, crowdsaleCap, isWeiAccepted, isNOTMinting);
        await whitelist.addManyToWhitelist([buyer]);
        await star.mint(buyer, 10e18);
        await token.mint(crowdsale.address, crowdsaleCap.mul(1e18));
        await token.unpause();
      ...
      it("transfers tokens up to crowdsale cap when buying with wei and sends remaining wei back to the buyer", async () => {
        await newCrowdsale(crowdsaleCap, crowdsaleCap, isWeiAccepted, isNOTMinting);
        await whitelist.addManyToWhitelist([buyer]);
        await token.mint(crowdsale.address, crowdsaleCap.mul(1e18));
        await token.unpause();
      ...
      it("only tranfers tokens up to crowdsale cap", async () => {
        await newCrowdsale(crowdsaleCap, crowdsaleCap, isWeiAccepted, isNOTMinting);
        await whitelist.addManyToWhitelist([buyer]);
        await token.mint(crowdsale.address, crowdsaleCap.mul(1e18));
        await star.mint(buyer, 10e18);
    ...
    describe("crowdsale finalization", function() {
      ...
      it("transfers remaining crowdsale tokens to wallet", async function() {
        const walletTokenBalance = await token.balanceOf(wallet);
